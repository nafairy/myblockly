...
/* 初始化Blockly块 */
SelectSortBlockly.initBlockly = function (blocklyDiv, mediaLocation) {
...
SelectSortExecute.setBlockly = function (blocklyDiv, mediaPath, XMLpath) {
    ...
    // 定义Blockly工作空间
    SelectSortBlockly.workspace = SelectSortBlockly.initBlockly(blocklyDiv, mediaPath);

    // 通过xml文件路径获取xml对象
    let workspaceXML = SelectSortBlockly.getWorkspaceXML(XMLpath);

    // 定义预定义块
    Blockly.Xml.domToWorkspace(workspaceXML, SelectSortBlockly.workspace);  
