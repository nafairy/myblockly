...
/* 初始化Blockly块 */
QuickSortBlockly.initBlockly = function (blocklyDiv, mediaLocation) {
...
QuickSortExecute.setBlockly = function (blocklyDiv, mediaPath, XMLpath) {
    ...
    // 定义Blockly工作空间
    QuickSortBlockly.workspace = QuickSortBlockly.initBlockly(blocklyDiv, mediaPath);

    // 通过xml文件路径获取xml对象
    let workspaceXML = QuickSortBlockly.getWorkspaceXML(XMLpath);

    // 定义预定义块
    Blockly.Xml.domToWorkspace(workspaceXML, QuickSortBlockly.workspace);  
