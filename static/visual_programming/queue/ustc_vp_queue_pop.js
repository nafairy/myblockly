...
/* 初始化Blockly块 */
QueuePopBlockly.initBlockly = function (blocklyDiv, mediaLocation) {
...
QueuePopExecute.setBlockly = function (blocklyDiv, mediaPath, XMLpath) {
    ...
    // 定义Blockly工作空间
    QueuePopBlockly.workspace = QueuePopBlockly.initBlockly(blocklyDiv, mediaPath);

    // 通过xml文件路径获取xml对象
    let workspaceXML = QueuePopBlockly.getWorkspaceXML(XMLpath);

    // 定义预定义块
    Blockly.Xml.domToWorkspace(workspaceXML, QueuePopBlockly.workspace);
