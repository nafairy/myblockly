...
/* 初始化Blockly块 */
LinkListInsertBlockly.initBlockly = function (blocklyDiv, mediaLocation) {
...
LinkListInsertExecute.setBlockly = function (blocklyDiv, mediaPath, XMLpath) {
    ...
    // 定义Blockly工作空间
    LinkListInsertBlockly.workspace = LinkListInsertBlockly.initBlockly(blocklyDiv, mediaPath);

    // 通过xml文件路径获取xml对象
    let workspaceXML = LinkListInsertBlockly.getWorkspaceXML(XMLpath);

    // 定义预定义块
    Blockly.Xml.domToWorkspace(workspaceXML, LinkListInsertBlockly.workspace);
